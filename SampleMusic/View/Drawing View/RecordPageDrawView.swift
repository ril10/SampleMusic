...
class RecordPageDrawView : UIView {
    ...
    var uploadLabel: UILabel = {
        let label = UILabel()
        label.text = "3.Create your sample"
        label.font = UIFont(name: Style.fontTitleHeavy.rawValue, size: 20.0)
        label.tintColor = .gray
        label.translatesAutoresizingMaskIntoConstraints = false
        return label
    ...
    func viewCompare(view: UIView) {
        ...
        NSLayoutConstraint.activate([
            ...
            createMusic.topAnchor.constraint(equalTo: uploadLabel.bottomAnchor),
            createMusic.centerXAnchor.constraint(equalTo: middleView.centerXAnchor),
            createMusic.bottomAnchor.constraint(equalTo: middleView.bottomAnchor),
